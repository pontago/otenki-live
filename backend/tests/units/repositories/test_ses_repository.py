...
def test_seed_loader(repository: SESRepository):
...
def test_get_template(repository: SESRepository):
...
def test_send_mail(repository: SESRepository):
    with pytest.raises(SESSendmailError):
        repository.sendmail(AppSettings.contact_from_address, "", "", "")
